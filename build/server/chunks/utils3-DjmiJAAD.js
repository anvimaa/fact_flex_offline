import { ah as sanitize_props, ai as spread_props, aj as rest_props, ak as fallback, al as attributes, am as clsx, an as ensure_array_like, ao as element, ap as slot, aq as bind_props, ar as clsx$1 } from './index-DPRpZFUH.js';
import { P as PUBLIC_BETTER_AUTH_URL } from './public-B844qK3e.js';
...
const baseURL = PUBLIC_BETTER_AUTH_URL;
...
function getStatusLabel(status) {
  const labels = {
    ...
    cancelado: "Cancelado",
    emitido: "Emitido",
    enviado_agt: "Enviado AGT",
    validado_agt: "Validado AGT",
    rejeitado_agt: "Rejeitado AGT",
...
function calculateTotalItem(item) {
  ...
  const taxRate = new Decimal(item.taxa);
  const discRate = new Decimal(item.desconto);
  const subtotal = qty.mul(price);
  const discount = subtotal.mul(discRate.div(100));
  const afterDiscount = subtotal.sub(discount);
  const tax = afterDiscount.mul(taxRate.div(100));
  return afterDiscount.add(tax).toNumber();
}
const calculateTotalDocumentoFiscal = (invoices) => {
  const total = invoices.reduce((acc, factura) => {
    const facturaTotal = factura.itens.reduce((sum, item) => {
      ...
      const afterDiscount = subtotal.sub(discount);
      const tax = afterDiscount.mul(taxRate.div(100));
      return sum.add(afterDiscount).add(tax);
    }, new Decimal(0));
    return acc.add(facturaTotal);
  }, new Decimal(0));
...
function getStatusBadge(status) {
  const badges = {
    ...
    EMITIDO: { variant: "default", icon: Circle_check, label: "Emitido" },
    ENVIADO_AGT: { variant: "default", icon: Clock, label: "Enviado" },
    VALIDADO_AGT: { variant: "default", icon: Circle_check, label: "Validado" },
    REJEITADO_AGT: { variant: "destructive", icon: Circle_x, label: "Rejeitado" },
    ANULADO: { variant: "destructive", icon: Circle_x, label: "Anulado" },
    RECTIFICADO: { variant: "secondary", icon: Circle_alert, label: "Rectificado" },
    PENDENTE_ENVIO: { variant: "secondary", icon: Clock, label: "Pendente" }
  };
  return badges[status] || { variant: "secondary", icon: Circle_alert, label: status };
}
function gerarCodigoVenda() {
  const agora = /* @__PURE__ */ new Date();
  const ano = String(agora.getFullYear()).slice(-2);
  const numeroSequencial = String(agora.getTime() % 1e6).padStart(6, "0");
  return `VE ${ano}/${numeroSequencial}`;
}
...
async function getCambio() {
  const cambio = await fetch("https://api.exchangerate-api.com/v4/latest/USD").then((response) => response.json()).then((data) => data.rates.AOA).catch((err) => {
    console.error(err);
    return 917.96;
  });
  return Number(cambio);
}
...
export { Clock as C, Decimal as D, Icon as I, Notebook_pen as N, Timer as T, calculateTotalItem as a, baseURL as b, calculateTotalDocumentoFiscal as c, getPrintUrl as d, gerarCodigoVenda as e, formatCurrency as f, getDocumentType as g, cn as h, extendTailwindMerge as i, getStatusBadgeVariant as j, Circle_check_big as k, Circle_x as l, Circle_alert as m, getStatusConfig as n, getStatusLabel as o, getRelativeDate as p, getTipoLabel as q, getStatusBadge as r, serialize as s, twMerge as t, Circle_check as u, getCambio as v, getFirstAndLastName as w, formatDate as x, formatDateTime as y, formatDates as z };
//# sourceMappingURL=utils3-DjmiJAAD.js.map
