import { r as requireMs, a as axios } from './index-Dn-KpEFr.js';
import require$$4 from 'https';
import { g as getDefaultExportFromCjs } from './_commonjsHelpers-BFTU3MAI.js';
import { r as requireSafeBuffer } from './api-BO0T-px8.js';
import stream from 'stream';
import require$$1 from 'util';
import crypto__default__default from 'crypto';
import require$$0 from 'buffer';
import { d as db } from './db-BUWqG89e.js';
import { g as getDocumentType, D as Decimal$1 } from './utils3-DjmiJAAD.js';
...
function requireVerify () {
	...
	const {KeyObject, createSecretKey, createPublicKey} = crypto__default__default;
...
function requireSign () {
	...
	const { KeyObject, createSecretKey, createPrivateKey } = crypto__default__default;
...
const BASE_URL_HML = "https://sifphml.minfin.gov.ao";
const BASE_URL_PROD = "https://sifp.minfin.gov.ao";
var Endpoints = /* @__PURE__ */ ((Endpoints2) => {
  Endpoints2["REGISTAR_FACTURA"] = "/sigt/fe/v1/registarFactura";
  Endpoints2["OBTER_ESTADO"] = "/sigt/fe/v1/obterEstado";
  Endpoints2["LISTAR_FACTURAS"] = "/sigt/fe/v1/listarFacturas";
  Endpoints2["CONSULTAR_FACTURA"] = "/sigt/fe/v1/consultarFactura";
  Endpoints2["SOLICITAR_SERIE"] = "/sigt/fe/v1/solicitarSerie";
  Endpoints2["LISTAR_SERIES"] = "/sigt/fe/v1/listarSeries";
  Endpoints2["VALIDAR_DOCUMENTO"] = "/sigt/fe/v1/validarDocumento";
  Endpoints2["LISTAR_SERIE"] = "/sigt/fe/v1/listarSeries";
  return Endpoints2;
})(Endpoints || {});
class AGTSigner {
  /**
   ...
   */
  static async sign(data, privateKeyPem) {
    const token = jwt.sign(data, privateKeyPem, { algorithm: "RS256" });
    return token;
  }
  /**
   ...
   */
  static async signSoftwareInfo(productId, productVersion, softwareValidationNumber, privateKeyPem, signatureVersion = 1) {
    const data = {
      productId,
      productVersion,
   ...
   */
  static async signRequest(payload, privateKeyPem) {
    return this.sign(payload, privateKeyPem);
  }
}
class AGTSDKError extends Error {
...
class AGTApiError extends AGTSDKError {
...
function calculateDocumentTotal(lines) {
  let acumuladorNetTotal = new Decimal$1(0);
  let acumuladorTaxPayable = new Decimal$1(0);
  const calculoLinhas = lines.map((line) => {
    const base = new Decimal$1(line.precoUnitario);
    const qtd = new Decimal$1(line.quantidade);
    const percDesc = new Decimal$1(line.desconto).div(100);
    const taxaIVA = new Decimal$1(line.taxa).div(100);
    const valorDescontoUnitario = base.mul(percDesc).toDecimalPlaces(2);
    const unitPrice = base.sub(valorDescontoUnitario);
    const settlementAmountTotal = valorDescontoUnitario.mul(qtd).toDecimalPlaces(2);
    const lineNetTotal = unitPrice.mul(qtd).toDecimalPlaces(2);
    const taxAmount = lineNetTotal.mul(taxaIVA).toDecimalPlaces(2);
    acumuladorNetTotal = acumuladorNetTotal.plus(lineNetTotal);
    acumuladorTaxPayable = acumuladorTaxPayable.plus(taxAmount);
    return {
      lineId: line.id,
      unitPriceBase: base.toDP(2).toNumber(),
      unitPrice: unitPrice.toDP(2).toNumber(),
      quantity: qtd.toNumber(),
      settlementAmount: settlementAmountTotal.toDP(2).toNumber(),
      taxPercentage: line.taxa.toNumber(),
      taxAmount: taxAmount.toDP(2).toNumber(),
      lineGrossTotal: lineNetTotal.plus(taxAmount).toDP(2).toNumber(),
      lineNetTotal: lineNetTotal.toDP(2).toNumber()
    };
  });
  const grossTotal = acumuladorNetTotal.plus(acumuladorTaxPayable);
  return {
    linesDocumentTotals: calculoLinhas,
    documentTotals: {
      netTotal: acumuladorNetTotal.toDP(2).toNumber(),
      taxPayable: acumuladorTaxPayable.toDP(2).toNumber(),
      grossTotal: grossTotal.toDP(2).toNumber()
    }
  };
}
...
class AGTClient {
  ...
  constructor(config) {
    ...
    if (!config.empresaPrivateKeyPem) {
      throw new Error("A chave privada da empresa não foi fornecida");
    }
    if (!config.taxRegistrationNumber) {
      throw new Error("O número de contribuinte da empresa não foi fornecido");
    }
    if (!config.productId) {
      throw new Error("O nome do Software não foi fornecido");
    }
    if (!config.productVersion) {
      throw new Error("A versão do Software não foi fornecida");
    }
    if (!config.softwareValidationNumber) {
      throw new Error("O número de validação do software não foi fornecido");
    }
    if (!config.environment) {
      throw new Error("O ambiente não foi fornecido");
    }
    if (!config.user) {
      throw new Error("O usuário não foi fornecido");
    }
    if (!config.password) {
      throw new Error("A senha não foi fornecida");
    }
    const baseURL = config.environment === "prod" ? BASE_URL_PROD : BASE_URL_HML;
   ...
   */
  async registerInvoices() {
    const submissionUUID = v4();
    ...
    const documentos = await db.documentoFiscal.findMany({
      where: {
        ...
        status: { in: ["EMITIDO", "PENDENTE_ENVIO"] }
      },
      include: {
        cliente: true,
        itens: {
    ...
    for (const doc of documentos) {
      const jwsDocumentSignature = await AGTSigner.signRequest(
        {
          documentNo: doc.numero,
          taxRegistrationNumber: this.config.taxRegistrationNumber,
          documentType: getDocumentType(doc.tipoDocumento),
          documentDate: doc.dataOperacao?.toISOString(),
          customerTaxID: doc.cliente?.nif,
          customerCountry: "AO",
          companyName: doc.cliente?.nome,
          documentTotals: {
            taxPayable: doc.totalImpostos.toString(),
            netTotal: doc.subtotal.toString(),
            grossTotal: doc.total.toString()
          }
        },
        this.config.privateKeyPem
      );
      const docum = {
        documentNo: doc.numero,
        documentStatus: doc.status === "EMITIDO" ? "N" : "A",
        companyName: doc.cliente?.nome,
        jwsDocumentSignature,
        documentDate: doc.dataEmissao.toISOString(),
        documentType: getDocumentType(doc.tipoDocumento),
        eacCode: doc.id.slice(0, 10),
        systemEntryDate: doc.dataOperacao?.toISOString(),
        customerTaxID: doc.cliente?.nif,
        customerCountry: "AO",
        lines: doc.itens.map((item, i) => {
          const taxa = item.produto?.taxa;
          ...
          return {
            ...
            unitPrice: item.precoUnitario.toNumber(),
            unitPriceBase: item.precoUnitario.toNumber(),
            settlementAmount: item.descontoValor.toNumber(),
            taxes: [
              {
                taxType: taxa ? taxa.tipo : "NS",
                taxCountryRegion: taxa ? taxa.regiao : "AO",
                taxCode: taxa ? taxa.codigo : "ISE",
                taxPercentage: taxa ? taxa.valor.toNumber() : 0,
                taxExemptionCode: taxa ? "" : item.motivoIsencao
              }
            ]
          };
        }),
        documentTotals: {
          taxPayable: doc.totalImpostos.toDP(2).toNumber(),
          netTotal: doc.subtotal.toDP(2).toNumber(),
          grossTotal: doc.total.toDP(2).toNumber()
        }
      };
      documents.push(docum);
    }
    const payload = {
      schemaVersion: "1.0",
      submissionUUID,
  ...
  async registerInvoice(invoice) {
    ...
    invoice.jwsDocumentSignature = jwsDocumentSignature;
  ...
  async registerInvoiceById(id) {
    const submissionUUID = v4();
    const submissionTimeStamp = (/* @__PURE__ */ new Date()).toISOString();
    const softwareInfo = await this.getSoftwareInfo();
    const doc = await db.documentoFiscal.findFirst({
      where: { id },
      include: { cliente: true, itens: { include: { produto: { include: { taxa: true } } } } }
    });
    if (!doc) {
      throw Error("Documento não encontrado. (Cliente)");
    }
    ...
    if (!allowedDocumentTypes.includes(doc.tipoDocumento)) {
      throw new Error("Apenas Facturas e Recibos podem ser submetidas a AGT.");
    }
    const jwsDocumentSignature = await AGTSigner.signRequest(
      {
        documentNo: doc.numero,
        taxRegistrationNumber: this.config.taxRegistrationNumber,
        documentType: getDocumentType(doc.tipoDocumento),
        documentDate: doc.dataEmissao?.toISOString().split("T"[0]),
        customerTaxID: doc.cliente?.nif,
        customerCountry: "AO",
        companyName: doc.cliente?.nome,
        documentTotals: {
    ...
    const invoice = {
      ...
      documentDate: doc.dataEmissao.toISOString().split("T")[0],
      documentType: getDocumentType(doc.tipoDocumento),
      //eacCode: doc.id.slice(0, 5),
      systemEntryDate: doc.createdAt?.toISOString(),
      customerCountry: "AO",
      customerTaxID: doc.cliente?.nif,
      companyName: doc.cliente?.nome,
      lines: doc.itens.map((item, i) => {
        const taxa = item.produto?.taxa;
        const lineNumber = i + 1;
        const taxContribution = taxa ? item.taxaValor.toNumber() : 0;
        const descRate = new Decimal(item.desconto);
        const priceBase = new Decimal(item.precoUnitario);
        const unitPriceBase = priceBase.mul(descRate.div(100));
        const unitPrice = priceBase.sub(unitPriceBase).toDecimalPlaces(2);
        return {
          lineNumber,
          operationType: "SG",
          productCode: item.codigo,
          ...
          taxes: [
            {
              taxType: taxa ? taxa.tipo : "IVA",
              taxCountryRegion: taxa ? taxa.regiao : "AO",
              taxCode: taxa ? taxa.codigo : "ISE",
              taxPercentage: taxa ? taxa.valor.toNumber() : 0,
              taxExemptionCode: taxa ? null : item.produto?.motivoIsento,
              // Calcular a taxa com
              taxContribution,
              taxAmount: taxContribution
            }
          ],
          settlementAmount: item.descontoValor.toNumber()
        };
    ...
    fs.writeFileSync(
      `./payloads/payload-${doc.numero.replaceAll("/", "-")}.json`,
    ...
    try {
      ...
      console.error("Erro no cliente ao enviar os dados na API:", error);
      this.handleError(error);
    }
  }
  /**
  ...
  async getInvoiceStatus(requestID) {
    ...
    try {
      const response = await this.axiosInstance.post(
        Endpoints.OBTER_ESTADO,
        data
      );
      return response.data;
    } catch (error) {
      console.error("Erro no Cliente. Consulta AGT:", error);
      this.handleError(error);
    }
  }
  /**
   ...
   */
  async listInvoices(queryStartDate, queryEndDate) {
    const softwareInfo = await this.getSoftwareInfo();
    const jwsSignature = await AGTSigner.signRequest(
      {
        taxRegistrationNumber: this.config.taxRegistrationNumber,
    ...
    const data = JSON.stringify(payload);
    try {
      const response = await this.axiosInstance.post(Endpoints.LISTAR_FACTURAS, data);
      return response.data;
    } catch (error) {
      this.handleError(error);
    }
   ...
   */
  async getInvoiceDetails(documentNo) {
    const softwareInfo = await this.getSoftwareInfo();
    const jwsSignature = await AGTSigner.signRequest(
      { taxRegistrationNumber: this.config.taxRegistrationNumber, documentNo },
      this.config.empresaPrivateKeyPem
    );
    const submissionUUID = v4();
    const submissionTimeStamp = (/* @__PURE__ */ new Date()).toISOString();
    ...
    const data = JSON.stringify(payload);
    try {
      const response = await this.axiosInstance.post(Endpoints.CONSULTAR_FACTURA, data);
      return response.data;
    } catch (error) {
      this.handleError(error);
    }
  ...
  async requestSeries(data) {
    ...
    const jwsSignature = await AGTSigner.signRequest(
      {
        ...
        seriesContingencyIndicator: data.seriesContingencyIndicator
      },
      this.config.empresaPrivateKeyPem
    );
    const submissionUUID = v4();
    ...
    try {
      const response = await this.axiosInstance.post(Endpoints.SOLICITAR_SERIE, dataSend);
      return response.data;
    } catch (error) {
      this.handleError(error);
    }
  ...
  /**
   * Lista as séries de faturação.
   */
  async listSeries(data) {
    const softwareInfo = await this.getSoftwareInfo();
    const jwsSignature = await AGTSigner.signRequest(
      {
        taxRegistrationNumber: this.config.taxRegistrationNumber
    ...
    const dataSend = JSON.stringify(payload);
    try {
      const response = await this.axiosInstance.post(Endpoints.LISTAR_SERIE, dataSend);
      return response.data;
    } catch (error) {
      this.handleError(error);
    }
  ...
  async registerInvoiceByIdi(id) {
    ...
    const typeCode = getDocumentType(doc.tipoDocumento);
    const allowedTypes = ["FA", "FT", "FR", "FG", "GF", "RC", "RG", "ND", "NC"];
    if (!allowedTypes.includes(typeCode)) {
      throw new Error(`Tipo de documento ${typeCode} não suportado para submissão.`);
    }
    const isRecibo = ["RC", "RG"].includes(typeCode);
    const isNotaCredito = typeCode === "NC";
    const { linesDocumentTotals, documentTotals } = calculateDocumentTotal(doc.itens);
    const jwsDocumentSignature = await AGTSigner.signRequest(
      {
        documentNo: doc.numero,
        taxRegistrationNumber: this.config.taxRegistrationNumber,
    ...
    const lines = isRecibo ? [] : doc.itens.map((item, i) => {
      const taxa = item.produto?.taxa;
      const atualLine = linesDocumentTotals.find((line2) => line2.lineId === item.id);
      const line = {
        lineNumber: i + 1,
        //operationType: 'SG', // Pode ser ajustado conforme a lógica de negócio [9]
        productCode: item.codigo,
        productDescription: item.descricao,
        quantity: item.quantidade,
        unitOfMeasure: item.unidade,
        unitPriceBase: atualLine?.unitPriceBase,
        unitPrice: atualLine?.unitPrice,
        debitAmount: isRecibo || isNotaCredito ? atualLine?.lineNetTotal : 0,
        creditAmount: isRecibo || isNotaCredito ? 0 : atualLine?.lineNetTotal,
        taxes: [
          {
            taxType: taxa ? taxa.tipo : "IVA",
            taxCountryRegion: taxa ? taxa.regiao : "AO",
            taxCode: taxa ? taxa.codigo : "ISE",
            taxPercentage: taxa ? taxa.valor.toDP(2).toNumber() : 0,
            taxExemptionCode: taxa ? void 0 : item.produto?.motivoIsento || "M00",
            taxContribution: atualLine?.taxAmount
            //taxAmount: atualLine?.taxAmount
          }
        ],
        settlementAmount: atualLine?.settlementAmount
      };
      if (isNotaCredito && doc.documentoOriginal) {
        line.referenceInfo = {
          reference: doc.documentoOriginal.numero,
          reason: doc.motivoEmissao || "Devolução de mercadoria",
          referenceItemLineNo: i + 1
        };
      }
      return line;
    });
    const paymentReceipt = isRecibo ? {
      sourceDocuments: [
        {
          lineNo: 1,
          sourceDocumentID: {
            originatingON: doc.documentoOriginal?.numero,
            documentDate: doc.documentoOriginal?.dataEmissao.toISOString().split("T")[0]
          },
          creditAmount: doc.documentoOriginal?.total.toDP(2).toNumber()
        }
      ]
    } : void 0;
    const invoice = {
      ...
      documentType: typeCode,
      systemEntryDate: doc.createdAt.toISOString(),
      // Formato YYYY-MM-DDThh:mm:ss [12]
      customerCountry: "AO",
      customerTaxID: doc.cliente?.nif || "999999999",
      companyName: doc.cliente?.nome,
      lines,
      paymentReceipt,
      documentTotals: {
        taxPayable: isRecibo ? 0 : documentTotals.taxPayable,
        netTotal: isRecibo ? 0 : documentTotals.netTotal,
        grossTotal: isRecibo ? doc.total.toDP(2).toNumber() : documentTotals.grossTotal
      }
    };
    const payload = {
      schemaVersion: "1.0",
    ...
    const data = JSON.stringify(payload);
    try {
      const response = await this.axiosInstance.post(Endpoints.REGISTAR_FACTURA, data);
      return response;
    } catch (error) {
      console.error("Erro no cliente ao enviar dados para a AGT:", error);
      this.handleError(error);
    }
  }
}

export { AGTClient as A };
//# sourceMappingURL=client3-D6-MZ0VN.js.map
