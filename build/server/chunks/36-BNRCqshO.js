...
const load = async ({ request }) => {
  ...
  const anoActual = (/* @__PURE__ */ new Date()).getFullYear().toString();
  ...
  const serieActiva = await db.serieAGT.findFirst({
    where: {
      empresaId,
      seriesStatus: { in: ["A", "U"] },
      seriesYear: anoActual,
      documentType: "FT"
    }
  });
  return {
    serie: serieActiva?.seriesCode,
    clientes,
...
const actions = {
  emitir: async ({ request }) => {
    ...
    const serie = data.get("serie")?.toString();
    if (!clienteId || !dataEmissao || !dataVencimento || !moeda || !itensJSON || !serie) {
    ...
    try {
      ...
      const docService = new DocumentoFiscalService();
      const documento = await docService.criarFactura({
        empresaId,
        clienteId,
        usuarioId,
      ...
      return fail(500, { error: error.message || "Erro ao emitir factura" });
    }
  }
};
