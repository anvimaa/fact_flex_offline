...
import { f as formatCurrency, D as Decimal } from './utils3-DjmiJAAD.js';
	...
	ParserAsync.prototype._inflateData = function (data) {
	  if (!this._inflate) {
	    if (this._bitmapInfo.interlace) {
	      ...
	      this._inflate = zlib$1.createInflate({ chunkSize: chunkSize });
...
function requireSyncInflate () {
	...
	(function (module, exports$1) {
		...
		util.inherits(Inflate, zlib$1.Inflate);
...
function ReciboTemplate($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    let { factura } = $$props;
    let documento = "Recibo";
    let subtotal = derived(() => factura.itens.reduce((acc, item) => acc.add(new Decimal(item.quantidade).mul(new Decimal(item.precoUnitario))), new Decimal(0)));
    let descontoTotal = derived(() => factura.itens.reduce(
      (acc, item) => {
        if (item.desconto) {
          const qty = new Decimal(item.quantidade);
          const price = new Decimal(item.precoUnitario);
          const disc = new Decimal(item.desconto);
          return acc.add(qty.mul(price).mul(disc).div(100));
        }
        return acc;
      },
      new Decimal(0)
    ));
    let totalCalculado = derived(() => subtotal().sub(descontoTotal()));
    ...
    async function handleExportPDF() {
      try {
        ...
        pdf.save(`recibo-${factura.numero}.pdf`);
      } catch (error) {
...
Segue a sua factura ${factura.numero} no valor de ${formatCurrency(totalCalculado())}.
...
${factura.empresa?.nome}`;
    ...
    $$renderer2.push(`<!----> <h2 class="hidden text-lg font-semibold text-gray-900 sm:block">Recibo #${escape_html(factura.numero)}</h2> <div class="flex items-center gap-2">`);
    Button($$renderer2, {
      variant: "outline",
      onclick: handlePrint,
      title: "Imprimir",
      children: ($$renderer3) => {
        $$renderer3.push(`<!---->Imprimir `);
    ...
    $$renderer2.push(`</div></div></div></div> <main class="flex-grow p-4 sm:p-8"><div class="invoice-content mx-auto max-w-4xl bg-white p-8 shadow-lg ring-1 ring-gray-200 print:p-0 print:shadow-none print:ring-0"><header class="flex items-start justify-between border-b pb-8"><div><img${attr("src", factura.empresa?.logo || "/default-logo.png")} alt="Logo da Empresa" class="h-12"/> <div class="mt-4 text-sm text-gray-600"><h1 class="text-lg font-bold text-gray-900">${escape_html(factura.empresa?.nome)}</h1> <p>${escape_html(factura.empresa?.endereco)}</p> <p>NIF: ${escape_html(factura.empresa?.nif)}</p></div></div> <div class="text-right"><h2 class="text-3xl font-bold uppercase text-gray-800">Recibo</h2> <p class="text-md text-gray-500">#${escape_html(factura.numero)}</p> `);
    ...
    $$renderer2.push(`<!--]--> <div class="space-y-1 text-sm"><p><span class="font-semibold text-gray-600">Data de Emissão:
      ...
      minute: "2-digit",
      second: "2-digit"
    }))}</span></p></div></div></header> <section class="mt-2 grid grid-cols-2 gap-8"><div><h3 class="text-sm font-semibold uppercase tracking-wider text-gray-500">Cliente</h3> <div class="mt-2 text-sm text-gray-800"><p class="font-bold">${escape_html(factura.cliente?.nome)}</p> <p>NIF: ${escape_html(factura.cliente?.nif)}</p> <p>${escape_html(factura.cliente?.endereco)}</p> <p>Contacto: ${escape_html(factura.cliente?.telefone)}</p></div></div></section> <section class="mt-4"><table class="w-full text-sm"><thead class="bg-gray-50"><tr><th class="px-4 py-3 text-left font-semibold uppercase tracking-wider text-gray-600">Documento</th><th class="px-4 py-3 text-right font-semibold uppercase tracking-wider text-gray-600">Data de Emissão</th><th class="px-4 py-3 text-right font-semibold uppercase tracking-wider text-gray-600">Total de Imposto</th><th class="px-4 py-3 text-right font-semibold uppercase tracking-wider text-gray-600">Total Facturado</th></tr></thead><tbody class="divide-y divide-gray-200"><tr><td class="px-4 py-4 text-gray-800">${escape_html(factura.referenciaOriginal)}</td><td class="px-4 py-4 text-right text-gray-600">${escape_html(factura.dataOperacao?.toLocaleDateString())}</td><td class="px-4 py-4 text-right text-gray-600">${escape_html(formatCurrency(factura.totalImpostos))}</td><td class="px-4 py-4 text-right text-gray-600">${escape_html(formatCurrency(factura.total))}</td></tr></tbody></table></section> <section class="mt-8 flex items-start justify-between"><div class="w-2/3 pr-8"><h3 class="text-sm font-semibold uppercase tracking-wider text-gray-500">Observações</h3> <div class="mt-2 text-sm text-gray-600">`);
    ...
    $$renderer2.push(`<!--]--> <p class="mt-4 italic">${escape_html(totalPorExtenso())}</p></div> `);
    {
      $$renderer2.push("<!--[!-->");
    }
    $$renderer2.push(`<!--]--></div> <div class="w-1/3"><div class="space-y-2 text-sm"><div class="flex justify-between"><span class="text-gray-600">Total de Impostos</span> <span class="font-medium text-gray-800">${escape_html(formatCurrency(factura.totalImpostos))}</span></div> <div class="flex justify-between"><span class="text-gray-600">Retenção/Cativação</span> <span class="font-medium text-gray-800">${escape_html(formatCurrency(descontoTotal()))}</span></div> <div class="mt-2 flex justify-between border-t pt-2"><span class="text-base font-bold text-gray-900">Total Pago</span> <span class="text-base font-bold text-gray-900">${escape_html(formatCurrency(factura.total))}</span></div></div></div></section> <footer class="mt-12 border-t pt-4 text-center text-xs text-gray-500"><p>${escape_html(factura.hashDocumento?.slice(0, 4))}-Processado por programa válido n31.1/AGT20</p> <p class="mt-1">Este documento serve de ${escape_html(documento.toLowerCase())}. Obrigado pela sua preferência!</p></footer></div></main></div>`);
  });
}
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    let { data } = $$props;
    head("1i4v0fo", $$renderer2, ($$renderer3) => {
      $$renderer3.title(($$renderer4) => {
        $$renderer4.push(`<title>FACT FLEXI | Recibo #${escape_html(data.factura?.numero)}</title>`);
      });
    });
    ReciboTemplate($$renderer2, { factura: data.factura, hash: data.hash });
  });
}

export { _page as default };
//# sourceMappingURL=_page.svelte-JAJQoOyo.js.map
