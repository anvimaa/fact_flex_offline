...
const load = async ({ request, url }) => {
  ...
  if (search) {
    where.OR = [{ numero: { contains: search } }, { cliente: { nome: { contains: search } } }];
  }
  const totalItems = await db.documentoFiscal.count({ where });
  ...
  await db.documentoFiscal.findMany({
    where: {
      tipoDocumento: { in: ["FACTURA", "FACTURA_RECIBO"] }
    }
  });
